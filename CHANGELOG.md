...
- **Health output reported a client's choice of name as the loaded model.**
  `runtime.model` was whatever model identifier the most recent request
  carried, and nothing at all before the first request arrived, so an operator
  console labelled "Model loaded" showed "none" while llama-server had the
  weights open, and then showed whatever string a caller had put in the field.
  Cordon spawns the supervised runtime against exactly one file, so it now
  reports that file from startup and a request cannot rename it. An external
  endpoint is a process Cordon did not spawn and cannot interrogate, so it
  still reports the last requested identifier, and `None` before the first
  request; that is the most that can honestly be said about a runtime someone
  else is running.
- **`cordon pull` could not complete a download.** The digest a pull checks
  against is `X-Linked-Etag`, which the Hub sets on its own response. The HEAD
  that read it followed the Hub's redirect to the storage backend, and on a
  Xet-backed repository, which is now most of them, that backend's `ETag` is the
  Xet content id: also exactly 64 hexadecimal characters, and not the file's
  SHA-256. Every pull therefore downloaded the right bytes, compared them
  against a value they could never equal, reported a digest mismatch, and
  deleted the model it had just fetched. The metadata request no longer follows
  redirects, and a plain `ETag` is trusted only when the Hub served the bytes
  itself.
...
Rebuilt around a persistent navigation rail and six views, posture,
attestation, audit chain, inference, model store, API surface, in place of five
flat tabs. Posture now leads with an **assurance ledger**: the four claims an
operator might make to an auditor, each with its own verdict and the consequence
of that verdict written out, so a node failing three of them cannot present a
single green light. The audit log is a filterable table whose rows expand to the
full entry hash and signature; the inference console carries a request inspector
holding the last response's token usage, finish reason, policy outcome and
signature provenance. Also: a `Ctrl-K` command palette, a Paper/Ink colour
scheme pair, a latency sparkline over the last sixty polls, a connection
indicator that backs off and visibly recedes the measurements when the node
stops answering, and table layouts that become record cards on a phone. Still
one file, still no webfont, still nothing fetched from the network.

The palette is a warm paper ground, graphite ink, hairline rules and square
corners, with exactly one saturated colour in the interface: moss green for
what the node has proven, ochre for what it has not, rust for what is broken.
Nothing else is allowed to be chromatic, so a coloured pixel always answers a
question about trust rather than about taste. The mark follows the same rule:
one line enters through the single break in a perimeter, turns once, and
closes around the one saturated pixel in the console.

The rail folds to its icons, either by the fold button in its footer or with
`Ctrl-\`, and the choice is remembered. Below 1180px the window folds it
anyway and the button is hidden, because a control that cannot hold its
setting should not be offered; the operator's preference is kept and applied
again when the window is widened. Folding it does not cost an operator the
ability to read it: each nav item now carries its own name, which it needed
regardless, since a button whose only label is `display: none` has no
accessible name at all.

**Fixed:** on a phone, every item in the navigation drawer was unreachable.
The drawer raises a scrim to dim the page behind it, and the scrim was stacked
above the drawer, so a tap meant for a nav item landed on the backdrop and
dismissed the menu instead. Every `z-index` in the console is now a token in
one ordered scale, which is the form in which this class of mistake is visible
at all.
