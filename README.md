...
# fingerprint against AWS's published value, and commit it, because the document
...
```bash
git clone https://github.com/regnant-io/cordon
cd cordon
cargo build --release
```
...
That digest is the `X-Linked-Etag` on the Hub's own response, and it is the only
value Cordon will accept. A large file's URL redirects to a storage backend, and
on a Xet-backed repository that backend's `ETag` is the Xet content id, which is
also 64 hexadecimal characters and is not the file's SHA-256. Cordon does not
follow the redirect when it asks for the digest, so it cannot mistake one for
the other.
...
consequence of that verdict in words. A node with three of them failing does not
get to show a green light.

The attestation view issues a fresh nonce, asks for a report bound to it, and
...
paste. The audit view shows the chain head, tails recent entries with their
hashes and signatures, verifies the whole chain, and takes an anchor.
