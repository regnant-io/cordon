...
//!
//! Recomputes the hash chain and checks every Ed25519 signature against the
//! log signing key's public half. It needs the exported log and that key, and
//! nothing else: it never contacts the node whose log it is checking.
//!
//! That independence is the point. When the key is derived from a Client Master
//! Key the client holds, a node cannot rewrite its own history without the
//! rewrite showing up here.
//!
//! ```text
//! cordon-verify-log --log <path> --key <hex> --deployment-id <id>
//! ```
//!
//! Exit status: `0` verified, `1` tampered or broken, `2` could not check.
...
fn run(cli: Cli) -> Result<bool> {
    // Parse verifying key
    let vk = VerifyingKey::from_hex(&cli.key).context("Invalid K_log verifying key hex")?;

    // Run verification
    let started = std::time::Instant::now();
    let result =
        verify_log_chain(&cli.log, &vk, &cli.deployment_id).context("Log verification failed")?;
