...
//! cordon-keygen derive      --cmk-file <path> --bundle-id <id> --client-id <id>
//! cordon-keygen show-public --cmk-file <path> --deployment-id <id> --client-id <id>
//! cordon-keygen admin-sign  --cmk-file <path> --deployment-id <id> --client-id <id> //!                           --action <action> --params <params>
//! ```
//!
//! In production the CMK belongs in an HSM. Writing it to a file is acceptable
//! only on a memory-backed filesystem that the node reads through
//! `CORDON_CMK_FILE`.
...
fn generate_keys(
    ...
    std::fs::write(&cmk_path, &cmk_hex).context("Cannot write CMK")?;
    ...
    let log_key = master
        .derive_log_key(&deployment_id, &client_id)
    ...
    println!(
        "Admin verification key (K_admin_pub): {}",
        admin_vk.to_hex()
    );
    ...
    if let Some(bid) = &bundle_id {
        let _bundle_key = master
            .derive_bundle_key(bid, &client_id)
...
fn derive_keys(
    ...
    match key_type.as_str() {
        "log" | "all" => {
            ...
            println!(
                "Log signing key (K_log pub):  {}",
                k.verifying_key().to_hex()
            );
        }
        _ => {}
    }
...
fn show_public_keys(cmk_hex: String, deployment_id: String, client_id: String) -> Result<()> {
    let master = MasterKey::from_hex(&cmk_hex).context("Invalid CMK hex")?;
