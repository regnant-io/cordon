...
use crate::{
    ...
    kdf::{derive_32_byte_key, bundle_key_info, shard_key_info, session_key_info, log_key_info, admin_key_info, enclave_key_info},
    signing::SigningKey,
    symmetric::AesGcmKey,
};
...
impl MasterKey {
    ...
    pub fn derive_session_key(&self, deployment_id: &str, client_id: &str) -> CryptoResult<SessionKey> {
    ...
    pub fn derive_log_signing_key(&self, deployment_id: &str, client_id: &str) -> CryptoResult<SigningKey> {
        let info = log_key_info(deployment_id, client_id);
        let bytes = derive_32_byte_key(&self.bytes, &info)?;
    ...
    pub fn derive_enclave_key(&self, deployment_id: &str, client_id: &str) -> CryptoResult<SigningKey> {
...
impl KeyHierarchy {
    ...
    pub fn new(master: MasterKey, deployment_id: String, client_id: String) -> Self {
        Self { master, deployment_id, client_id }
    }
    ...
    pub fn derive_all(&self, bundle_id: &str) -> CryptoResult<DerivedKeys> {
        Ok(DerivedKeys {
            ...
            session_key: self.master.derive_session_key(&self.deployment_id, &self.client_id)?,
            log_key: self.master.derive_log_key(&self.deployment_id, &self.client_id)?,
            admin_key: self.master.derive_admin_key(&self.deployment_id, &self.client_id)?,
        })
    }
}
...
mod tests {
    ...
    fn test_key_derivation_deterministic() {
        ...
        let k2 = test_cmk().derive_bundle_key("bundle-1", "client-a").unwrap();
    ...
    fn test_different_bundles_different_keys() {
        let cmk = test_cmk();
        let k1 = cmk.derive_bundle_key("bundle-1", "client-a").unwrap();
        let k2 = test_cmk().derive_bundle_key("bundle-2", "client-a").unwrap();
    ...
    fn test_different_purposes_different_keys() {
        ...
        let session_key = test_cmk().derive_session_key("deploy-1", "client-a").unwrap();
