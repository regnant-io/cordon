...
pub enum CordonError {
    ...
    /// Every concurrency slot is occupied. The caller should retry later.
    #[error("Node is at capacity ({max_concurrent} concurrent requests) — retry shortly")]
    Overloaded {
        /// Configured concurrency ceiling.
        max_concurrent: u32,
    },

    /// The model runtime is missing, unreachable, or refused to start.
    #[error("Model runtime unavailable: {0}")]
    RuntimeUnavailable(String),

    /// A model could not be fetched from the Hub.
    #[error("Model download failed: {0}")]
    ModelDownloadFailed(String),

    /// The request body exceeded a configured limit.
    #[error("Request rejected: {0}")]
    RequestTooLarge(String),

    /// The operation is not permitted in this deployment mode.
    #[error("Not permitted in {mode} mode: {reason}")]
    ModeForbidden {
...
impl CordonError {
    /// Whether the error reflects a caller mistake rather than a node fault.
    /// Used to decide whether a failure counts toward attack detection.
    pub fn is_client_fault(&self) -> bool {
        matches!(
            self,
            CordonError::AuthFailed(_)
                | CordonError::ValidationFailed(_)
                | CordonError::RateLimitExceeded { .. }
                | CordonError::ModelNotFound { .. }
                | CordonError::ContentPolicyViolation { .. }
                | CordonError::RequestTooLarge(_)
                | CordonError::AdminRejected(_)
        )
    }
}
