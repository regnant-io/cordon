...
//!
//! The choice also decides how much health output can honestly say. Cordon
//! spawned the supervised runtime against one model file, so it reports that
//! model from startup. It did not spawn an external endpoint and cannot ask
//! that process what it loaded, so it reports the model identifier carried by
//! the most recent accepted request, and nothing at all before the first one.
//! See [`InferenceBackend::loaded_model`].
