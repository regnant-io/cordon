...
impl CovertChannelDetector {
    ...
    fn score_patterns(&self, text: &str, findings: &mut Vec<String>) -> f32 {
        ...
        if b64_density > self.config.base64_density_threshold {
            findings.push(format!("Base64-like content density: {:.1}%", b64_density * 100.0));
            max_score = max_score.max(b64_density.min(1.0));
        }

        // Check hex density
        let hex_density = self.compute_contiguous_density(text, |c| {
            c.is_ascii_hexdigit()
        });
        if hex_density > self.config.hex_density_threshold {
            findings.push(format!("Hex-like content density: {:.1}%", hex_density * 100.0));
            max_score = max_score.max(hex_density.min(1.0));
        }

        // Check for numeric-dense patterns (potential encoded data)
        let digit_fraction = text.chars().filter(|c| c.is_ascii_digit()).count() as f32 / total_chars;
        if digit_fraction > 0.30 {
            findings.push(format!("High numeric density: {:.1}%", digit_fraction * 100.0));
    ...
    fn score_whitespace_patterns(&self, text: &str, findings: &mut Vec<String>) -> f32 {
        ...
        let suspicious_whitespace = text.chars()
            .filter(|c| {
                matches!(c,
...
mod tests {
    ...
    fn test_normal_text_low_score() {
        ...
        assert!(result.anomaly_score < 0.4, "Normal text anomaly score too high: {}", result.anomaly_score);
    }
    ...
    fn test_base64_heavy_text_triggers() {
        ...
        assert!(result.pattern_score > 0.1, "Base64-heavy content should elevate pattern score");
    }
