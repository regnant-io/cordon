...
impl NodeState {
    ...
    /// quarantine, locked, or zeroized — those are stronger states that a
    /// transient fault must not silently relax.
    pub fn degrade(&mut self, reason: &str) {
        if matches!(self.status, NodeStatus::Healthy | NodeStatus::Degraded) {
            self.status = NodeStatus::Degraded;
            tracing::warn!(reason, "Node degraded");
        }
    }
}
...
impl SharedNodeState {
    ...
    pub fn update_latency(&self, latency_ms: u64) {
        ...
        if state.stats.latency_ms_p50 == 0 {
            ...
            state.stats.latency_ms_p50 = (state.stats.latency_ms_p50 * 9 + latency_ms) / 10;
