...
pub trait InferenceBackend: Send + Sync {
    ...
    ///
    /// A backend whose process Cordon spawned answers from the model it was
    /// launched with, and can answer before the first request. A backend Cordon
    /// does not own cannot be asked what it loaded, so it reports the
    /// identifier on the most recent accepted request, and `None` until one
    /// arrives. [`InferenceBackend::backend_name`] is what tells the two apart.
